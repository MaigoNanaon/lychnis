...
- **播放页面**：类 Apple Music 风格，旋转唱片封面 + 播放控制 + 单人/双人模式入口
- **影子对决**：
  - 先选择歌曲，再从播放页进入双人模式，至少展示 3 秒匹配过程
  - 只匹配同歌曲、同谱面版本的预录节奏影子
  - 横屏上下双轨对决，共用同一歌曲时钟
  - 实时展示双方分数、连击、领先状态和分差
...
1. **选择歌曲**：在曲库点击歌曲，进入该歌曲的播放页
2. **开始匹配**：点击右侧「双人Q-beat！」，本局曲目将锁定为当前歌曲
3. **等待对手**：匹配动画至少持续 3 秒，同时预加载当前歌曲
4. **确认对手**：查看对手资料、能力值和已锁定的本局曲目
5. **开始对决**：进入横屏上下双轨，上方为影子，下方为玩家
6. **跟随节奏**：轻点下半屏打击，影子会按历史点击时间线同步回放
7. **查看胜负**：歌曲结束后比较总分、准确率和最大连击
8. **继续挑战**：可复用同一影子再来一局，或在同一首歌下重新匹配
